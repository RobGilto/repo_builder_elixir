...
| The design system — the stack-aware UI component vocabulary/tokens/rules per surface (web/TUI), the `:design_system` kind, `DesignResolver`/`DesignContract`, the `resolve_design_system` tool, surface/framework detection, or adding a framework's design system | `ai_docs/design-system-plugins.md` (authoring a design system) + `specs/tui-framework-decision-reference.md` (the verified TUI framework/component knowledge base) |
...
| planf3 HTML plans — authoring (`/plan_f3`), executing/updating a `specs/*.html` plan, the placeholder-image library, or `PLANF3_IMAGES`/`ADW_PLAN_COMMAND` | `.claude/commands/plan_f3.md` (the planner command) + `ai_docs/planf3/` (lifecycle workflows) + `specs/planf3-html-plans-for-heavy-adw-planner.html` (the incorporation spec) |
