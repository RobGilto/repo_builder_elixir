...
## The five stages (Ruby)

| Stage | Command | Strict? | Cadence |
|-------|---------|---------|---------|
| format | `rubocop` | yes | per phase |
| lint | `rubocop` | yes | per phase |
| type | `srb tc (Sorbet)` | yes | per phase |
| test | `rspec` | yes | per phase |
| mutation | `mutant` | advisory | **pre-merge (close)** |
...
Property tests (**PropCheck**) are the other spec-encoding lever: assert invariants drawn
