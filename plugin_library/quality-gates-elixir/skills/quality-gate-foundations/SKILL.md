...
## The five stages (Elixir)

| Stage | Command | Strict? | Cadence |
|-------|---------|---------|---------|
| format | `mix format --check-formatted` | yes | per phase |
| lint | `mix credo --strict` | yes | per phase |
| type | `mix dialyzer (+ Credo @spec gate)` | yes | per phase |
| test | `mix test --warnings-as-errors` | yes | per phase |
| mutation | `muzak` | advisory | **pre-merge (close)** |
...
Property tests (**StreamData**) are the other spec-encoding lever: assert invariants drawn
