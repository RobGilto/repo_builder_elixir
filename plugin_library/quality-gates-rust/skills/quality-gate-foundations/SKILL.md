...
## The five stages (Rust)

| Stage | Command | Strict? | Cadence |
|-------|---------|---------|---------|
| format | `cargo fmt --check` | yes | per phase |
| lint | `cargo clippy -- -D warnings` | yes | per phase |
| type | `cargo check (RUSTFLAGS="-D warnings")` | yes | per phase |
| test | `cargo nextest run` | yes | per phase |
| mutation | `cargo-mutants` | advisory | **pre-merge (close)** |
...
Property tests (**proptest**) are the other spec-encoding lever: assert invariants drawn
