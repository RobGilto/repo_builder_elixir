...
|-------|---------|---------|---------|
| format | `gofmt -l .` | yes | per phase |
| lint | `golangci-lint run` | yes | per phase |
| type | `go build ./... + staticcheck` | yes | per phase |
| test | `go test -race -shuffle=on -count=1 ./...` | yes | per phase |
| mutation | `go-mutesting` | advisory | **pre-merge (close)** |
...
Property tests (**gopter / rapid**) are the other spec-encoding lever: assert invariants drawn
