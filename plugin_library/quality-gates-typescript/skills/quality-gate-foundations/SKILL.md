...
## The five stages (TypeScript)

| Stage | Command | Strict? | Cadence |
|-------|---------|---------|---------|
| format | `biome format --check` | yes | per phase |
| lint | `biome lint` | yes | per phase |
| type | `tsc --noEmit + strict tsconfig` | yes | per phase |
| test | `vitest run` | yes | per phase |
| mutation | `Stryker` | advisory | **pre-merge (close)** |
...
Property tests (**fast-check**) are the other spec-encoding lever: assert invariants drawn
