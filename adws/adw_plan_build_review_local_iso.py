...
"""
ADW Plan Build Review Local Iso - GitHub-optional plan + build + review in an
isolated worktree

Usage:
  uv run adw_plan_build_review_local_iso.py <adw-id> [--skip-resolution]
...
Local-mode simplifications: the review resolution loop is disabled — review
blockers are narrated as warnings (non-fatal). The --skip-resolution flag is
...
6. Review step: /review single pass (agent: reviewer) — blockers non-fatal;
...
WORKFLOW_NAME = "adw_plan_build_review_local_iso"
...
def main():
    ...
    # Accepted for CLI compatibility; resolution is always skipped locally.
    if "--skip-resolution" in sys.argv:
        sys.argv.remove("--skip-resolution")

    if len(sys.argv) < 2:
        print("Usage: uv run adw_plan_build_review_local_iso.py <adw-id> [--skip-resolution]")
    ...
    logger.info(f"ADW Plan Build Review Local Iso starting - ID: {adw_id}")
    ...
    narrate(adw_id, "✅ Starting local plan+build+review (GitHub-optional)")
    ...
    maybe_comment_on_real_issue(
        run, f"{adw_id}_ops: ✅ Local plan+build+review started in {worktree_path}", logger
    )
    ...
    local_ops.update_run(
        ...
        output={"commit": review_commit or commit_sha},
    )
    state.save(WORKFLOW_NAME)
    narrate(adw_id, "✅ Local plan+build+review completed")
    maybe_comment_on_real_issue(
        run,
        f"{adw_id}_ops: ✅ Local plan+build+review completed — spec {spec_file}, "
        f"branch {branch_name}, {len(blockers)} review blocker(s)",
        logger,
    )
    logger.info("Local plan+build+review completed successfully")
