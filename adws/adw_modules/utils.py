...
def get_safe_subprocess_env() -> Dict[str, str]:
    ...
    safe_env_vars = {
        ...
        # Harness / provider abstraction knobs — pass the operator selection
        # through to the spawned harness subprocess.
        "ADW_HARNESS": os.getenv("ADW_HARNESS"),
        "ADW_PROVIDER": os.getenv("ADW_PROVIDER"),
        "ADW_MODEL_FAST": os.getenv("ADW_MODEL_FAST"),
        "ADW_MODEL_MAIN": os.getenv("ADW_MODEL_MAIN"),
        "ADW_MODEL_HEAVY": os.getenv("ADW_MODEL_HEAVY"),

        # pi coding agent configuration (when ADW_HARNESS=pi)
        "PI_CODING_AGENT_PATH": os.getenv("PI_CODING_AGENT_PATH"),
        "PI_BIN": os.getenv("PI_BIN"),
        "PI_CODING_AGENT_DIR": os.getenv("PI_CODING_AGENT_DIR"),
        "PI_OFFLINE": os.getenv("PI_OFFLINE"),
    ...
    # Include API key only if set — omitting it lets Claude Code fall back to claude.ai auth
    api_key = os.getenv("ANTHROPIC_API_KEY")
    if api_key:
        safe_env_vars["ANTHROPIC_API_KEY"] = api_key

    # Provider credentials for non-anthropic harness/provider pairs — included
    # only when set so they never override Claude Code's claude.ai auth fallback.
    for provider_key in ("OPENAI_API_KEY", "ZAI_API_KEY"):
        value = os.getenv(provider_key)
        if value:
            safe_env_vars[provider_key] = value
