...
def create_worktree(adw_id: str, branch_name: str, logger: logging.Logger) -> Tuple[str, Optional[str]]:
    ...
    for base in bases:
        ...
        if result.returncode == 0:
            logger.info(f"Created worktree from base: {base}")
            break
        ...
        if "already exists" in result.stderr:
            ...
            result = subprocess.run(cmd, capture_output=True, text=True, cwd=project_root)
            break
        logger.warning(f"Base {base} unavailable for worktree: {result.stderr.strip()}")

    if result is None or result.returncode != 0:
        error_msg = f"Failed to create worktree: {result.stderr if result else 'no base branch found'}"
