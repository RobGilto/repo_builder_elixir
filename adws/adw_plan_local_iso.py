...
"""
ADW Plan Local Iso - GitHub-optional plan-only workflow in an isolated worktree

Usage:
  uv run adw_plan_local_iso.py <adw-id>
...
5. Commit the spec in the worktree (no push); mark completed
...
WORKFLOW_NAME = "adw_plan_local_iso"
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: uv run adw_plan_local_iso.py <adw-id>")
    ...
    logger.info(f"ADW Plan Local Iso starting - ID: {adw_id}")
    ...
    narrate(adw_id, "✅ Starting local plan (GitHub-optional)")
    ...
    maybe_comment_on_real_issue(
        run, f"{adw_id}_ops: ✅ Local plan started in {worktree_path}", logger
    )
    ...
    # Commit the spec in the worktree (no push — GitHub stays optional)
    commit_msg, error = create_commit(
        AGENT_PLANNER, issue, ISSUE_CLASS, adw_id, logger, worktree_path
    )
    if error:
        local_ops.step_end(adw_id, "plan", "failed")
        fail(adw_id, "plan", f"Error creating commit message: {error}", logger)

    success, error = commit_changes(commit_msg, cwd=worktree_path)
    if not success:
        local_ops.step_end(adw_id, "plan", "failed")
        fail(adw_id, "plan", f"Error committing plan: {error}", logger)
    ...
    state.save(WORKFLOW_NAME)
    narrate(adw_id, "✅ Local plan completed")
    maybe_comment_on_real_issue(
        run,
        f"{adw_id}_ops: ✅ Local plan completed — spec {spec_file}, "
        f"branch {branch_name}, commit {commit_sha or 'n/a'}",
        logger,
    )
    logger.info("Local plan completed successfully")
