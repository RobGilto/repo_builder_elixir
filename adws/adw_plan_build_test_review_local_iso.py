...
"""
ADW Plan Build Test Review Local Iso - GitHub-optional plan + build + test +
review in an isolated worktree

Usage:
  uv run adw_plan_build_test_review_local_iso.py <adw-id> [--skip-e2e] [--skip-resolution]
...
WORKFLOW_NAME = "adw_plan_build_test_review_local_iso"
...
def main():
    ...
    # Accepted for CLI compatibility; in local mode E2E and resolution loops
    # are always skipped regardless of these flags.
    for flag in ("--skip-e2e", "--skip-resolution"):
        if flag in sys.argv:
            sys.argv.remove(flag)

    if len(sys.argv) < 2:
        print("Usage: uv run adw_plan_build_test_review_local_iso.py <adw-id> [--skip-e2e] [--skip-resolution]")
    ...
    logger.info(f"ADW Plan Build Test Review Local Iso starting - ID: {adw_id}")
    ...
    narrate(adw_id, "✅ Starting local plan+build+test+review (GitHub-optional)")
    ...
    maybe_comment_on_real_issue(
        run,
        f"{adw_id}_ops: ✅ Local plan+build+test+review started in {worktree_path}",
        logger,
    )
    ...
    state.save(WORKFLOW_NAME)
    narrate(adw_id, "✅ Local plan+build+test+review completed")
    maybe_comment_on_real_issue(
        run,
        f"{adw_id}_ops: ✅ Local plan+build+test+review completed — spec {spec_file}, "
        f"branch {branch_name}, tests {passed_count}p/{failed_count}f, "
        f"{len(blockers)} review blocker(s)",
        logger,
    )
    logger.info("Local plan+build+test+review completed successfully")
