...
"""
ADW Plan Build Test Local Iso - GitHub-optional plan + build + test in an
isolated worktree

Usage:
  uv run adw_plan_build_test_local_iso.py <adw-id> [--skip-e2e]
...
Local-mode simplifications (prompt-mode runs have no browser context): E2E
tests are always skipped and the test resolution loop is disabled — test
failures are narrated as warnings (non-fatal gate, mirroring adw_sdlc_iso's
test phase). The --skip-e2e flag is accepted for CLI compatibility.
...
WORKFLOW_NAME = "adw_plan_build_test_local_iso"
...
def run_test_phase(
    ...
    Returns (results, passed_count, failed_count); failed_count of -1 is the
    agent-level-error sentinel (caller fails the step)."""
...
def main():
    ...
    # Accepted for CLI compatibility; E2E is always skipped in local mode.
    if "--skip-e2e" in sys.argv:
        sys.argv.remove("--skip-e2e")

    if len(sys.argv) < 2:
        print("Usage: uv run adw_plan_build_test_local_iso.py <adw-id> [--skip-e2e]")
    ...
    logger.info(f"ADW Plan Build Test Local Iso starting - ID: {adw_id}")
    ...
    narrate(adw_id, "✅ Starting local plan+build+test (GitHub-optional)")
    ...
    maybe_comment_on_real_issue(
        run, f"{adw_id}_ops: ✅ Local plan+build+test started in {worktree_path}", logger
    )
    ...
    state.save(WORKFLOW_NAME)
    narrate(adw_id, "✅ Local plan+build+test completed")
    maybe_comment_on_real_issue(
        run,
        f"{adw_id}_ops: ✅ Local plan+build+test completed — spec {spec_file}, "
        f"branch {branch_name}, tests {passed_count}p/{failed_count}f",
        logger,
    )
    logger.info("Local plan+build+test completed successfully")
