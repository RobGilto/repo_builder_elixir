...
"""
ADW Plan Build Document Local Iso - GitHub-optional plan + build + document in
an isolated worktree

Usage:
  uv run adw_plan_build_document_local_iso.py <adw-id>
...
6. Document step: /document (agent: documenter); commit; mark completed
...
WORKFLOW_NAME = "adw_plan_build_document_local_iso"
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: uv run adw_plan_build_document_local_iso.py <adw-id>")
    ...
    logger.info(f"ADW Plan Build Document Local Iso starting - ID: {adw_id}")
    ...
    narrate(adw_id, "✅ Starting local plan+build+document (GitHub-optional)")
    ...
    maybe_comment_on_real_issue(
        run,
        f"{adw_id}_ops: ✅ Local plan+build+document started in {worktree_path}",
        logger,
    )
    ...
    local_ops.update_run(
        ...
        output={"commit": doc_commit or commit_sha},
    )
    state.save(WORKFLOW_NAME)
    narrate(adw_id, "✅ Local plan+build+document completed")
    maybe_comment_on_real_issue(
        run,
        f"{adw_id}_ops: ✅ Local plan+build+document completed — spec {spec_file}, "
        f"branch {branch_name}, commit {doc_commit or commit_sha or 'n/a'}",
        logger,
    )
    logger.info("Local plan+build+document completed successfully")
