...
- **Typed coding standard (enforced):** all work must follow `ai_docs/typed-elixir-standard.md` — an `@spec` on every public function (`@impl` callbacks exempt), `typedstruct`/`@enforce_keys` structs, precise types over `any()`/`map()`, tagged-tuple returns over raising, and wire-type ≠ domain-type at every boundary. It is gated by `--warnings-as-errors`, `mix credo --strict` (`Credo.Check.Readability.Specs`), and `mix dialyzer`. The frozen origin is `BUILD_PROMPT.md` §3.
- The authoritative architecture/spec is `BUILD_PROMPT.md`. `.claude/commands/conditional_docs.md` routes tasks to the relevant reference docs.
